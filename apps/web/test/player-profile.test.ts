...
function insertUser(client: CuatroClient, email: string, displayName: string, extra: Record<string, unknown> = {}) {
  return client.db.insert(users).values({ email, displayName, ...extra }).returning().get();
}

function insertCircleAndSession(client: CuatroClient, createdBy: string, startsAt: Date) {
  const circle = client.db
    .insert(circles)
    .values({ name: "Test Circle", inviteCode: `INV-${Math.random().toString(36).slice(2, 10)}`, createdBy })
    .returning()
    .get();
  const session = client.db.insert(sessions).values({ circleId: circle.id, startsAt, status: "played" }).returning().get();
...
describe("public player profile read model", () => {
  ...
  it("reports a guest as isGuest with no rating", async () => {
    const guest = insertUser(h.client, `g-${Math.random()}@x.com`, "Casey Guest", { isGuest: true });
  ...
  it("assembles a rated player's Glass, history, ledger, and circles in common", async () => {
    const viewer = insertUser(h.client, "viewer@x.com", "Vic Viewer");
    const target = insertUser(h.client, "target@x.com", "Tara Target");
    const partner = insertUser(h.client, "partner@x.com", "Pat Partner");

    // A shared circle → circlesInCommon = 1.
    const shared = h.client.db
      .insert(circles)
      .values({ name: "Shared", inviteCode: `INV-shared-${Math.random().toString(36).slice(2, 8)}`, createdBy: viewer.id })
      .returning()
      .get();
    h.client.db.insert(circleMembers).values({ circleId: shared.id, userId: viewer.id, role: "organiser" }).run();
    h.client.db.insert(circleMembers).values({ circleId: shared.id, userId: target.id, role: "member" }).run();

    // Three verified wins → target crosses the Placement Trio and becomes rated.
    const start = Date.now() - 10 * DAY_MS;
    for (let i = 0; i < 3; i++) {
      const opp1 = insertUser(h.client, `o1-${i}@x.com`, `Opp1-${i}`);
      const opp2 = insertUser(h.client, `o2-${i}@x.com`, `Opp2-${i}`);
      const { sessionId } = insertCircleAndSession(h.client, target.id, new Date(start + i * DAY_MS));
  ...
  it("circlesInCommon is null when the viewer is the player themselves", async () => {
    const solo = insertUser(h.client, "solo@x.com", "Solo");
